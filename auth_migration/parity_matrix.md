...
| POST /auth/oauth/introspect | ✅ | RFC 7662. Caller auth: client-credentials (Basic or params) or a bearer token. No credentials → **400 invalid_request**; bad client creds → 401 invalid_client; bad bearer → 401 invalid_token. Visibility matches Doorkeeper's default: a client-credential caller sees another app's token as `{"active":false}`; a bearer caller may only introspect its own application's tokens and **not** the token it authenticated with (both → 401 invalid_token). 401s carry `WWW-Authenticate: Bearer realm="Doorkeeper"` + `Cache-Control: no-store` |
...
4. **Discovery capability lists**: honest advertisement vs Rails' aspirational one. `scopes_supported` is a curated set (`openid profile email offline_access public`) rather than Rails' full ~90-entry PlaceOS-scope enumeration; `code_challenge_methods_supported`/grant/response-type lists reflect what auth.cr actually supports. Load-bearing endpoint URLs + `jwks_uri`/`introspection_endpoint` match exactly.
...
6. **Authorize endpoint errors**: redirectable OAuth errors (e.g. `invalid_scope`, `unsupported_response_type`) return a JSON 400 rather than Doorkeeper's 302 back to `redirect_uri?error=…`. This is pre-existing behaviour of the GET authorize handler (shared by the new POST alias), not introduced here; non-redirectable errors and the success path match.
7. **Application record timestamps**: Doorkeeper's application JSON included `created_at`/`updated_at`; the `oauth_applications` table has no timestamp columns, so auth.cr omits them.
